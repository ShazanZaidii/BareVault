...
const (
    CloudflareGatewayURL = "your_cloudflare_gateway_url_here" 
    HMACSecret           = "your_hmac_secret_key" // Must match worker.js
)
...
func prepareEnvironment() (string, error) {
    ...
    for _, name := range required {
        ...
        if _, err := os.Stat(path); os.IsNotExist(err) {
            err := downloadBinary(name, binDir)
            if err != nil {
...
func downloadBinary(name, targetDir string) error {
    ...
    for _, url := range urls {
        ...
        if resp.StatusCode >= 200 && resp.StatusCode < 300 {
            activeURL = resp.Request.URL.String()
            break
        }
    ...
    hashURL := activeURL + ".sha256"
    if name == "minio" {
        hashURL = activeURL + ".sha256sum"
    }
    expectedHash := getExpectedHash(hashURL)
    ...
    if name == "cloudflared" && osName == "darwin" {
        ...
        if expectedHash != "" && !verifyChecksum(archivePath, expectedHash) {
            os.Remove(archivePath)
            return fmt.Errorf("security alert: checksum mismatch for cloudflared archive")
        }
    ...
    os.Remove(tempDest)
    out, _ := os.Create(tempDest)
    pw := &ProgressWriter{Total: resp.ContentLength, FileName: name}
    io.Copy(out, io.TeeReader(resp.Body, pw))
    out.Close()
    fmt.Println()

    if expectedHash != "" && !verifyChecksum(tempDest, expectedHash) {
        os.Remove(tempDest)
        return fmt.Errorf("security alert: checksum mismatch for %s", name)
    }
...
func startAdminAPI(port string, apiPort string, profile VaultProfile) {
    ...
    mux.HandleFunc("/api/invite/redeem", func(w http.ResponseWriter, r *http.Request) {
        ...
        // Generate IAM Policy based on invite settings
...
func getExpectedHash(url string) string {
